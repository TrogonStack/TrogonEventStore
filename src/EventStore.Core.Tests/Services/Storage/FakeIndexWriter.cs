...
public class FakeIndexWriter<TStreamId> : IIndexWriter<TStreamId>
{
	...
	public ValueTask<CommitCheckResult<TStreamId>> CheckCommitStartingAt(long transactionPosition, long commitPosition,
		CancellationToken token) =>
		ValueTask.FromResult(new CommitCheckResult<TStreamId>(CommitDecision.Ok, GetFakeStreamId(), -1, -1, -1, false));

	public ValueTask<CommitCheckResult<TStreamId>> CheckCommit(TStreamId streamId, long expectedVersion,
		IEnumerable<Guid> eventIds, bool streamMightExist, CancellationToken token) =>
		ValueTask.FromResult(new CommitCheckResult<TStreamId>(CommitDecision.Ok, streamId, expectedVersion, -1, -1,
			false));
