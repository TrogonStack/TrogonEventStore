	...
	: ReadIndexTestScenario<TLogFormat, TStreamId>
{
	...
	protected override async ValueTask WriteTestScenario(CancellationToken token)
	{
		...
		await WriteSingleEvent(_disallowedStream, 1, new string('.', 3000), eventId: Guid.NewGuid(),
			eventType: "event-type-2",
			retryOnFail: true, token: token); //disallowed
		await WriteSingleEvent(_allowedStream2, 1, new string('.', 3000), eventId: Guid.NewGuid(),
