...
public class
	when_having_multiple_metaevents_in_metastream_and_read_index_is_set_to_keep_last_2<TLogFormat, TStreamId>() :
	SimpleDbTestScenario<TLogFormat, TStreamId>(metastreamMaxCount: 2)
{
	...
	public async Task single_event_read_returns_last_two_events()
	{
		...
		Assert.AreEqual(ReadEventResult.NotFound,
			(await ReadIndex.ReadEvent("$$test", 2, CancellationToken.None)).Result);
