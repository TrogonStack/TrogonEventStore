...
public class
	when_writing_a_second_event_after_the_first_event_has_been_replicated<TLogFormat, TStreamId> :
	WriteEventsToIndexScenario<TLogFormat, TStreamId>
{
	...
	public override async ValueTask WriteEvents(CancellationToken token)
	{
		...
		var prepares =
			CreatePrepareLogRecord(_streamId, expectedEventNumber, eventTypeId, _eventId, transactionPosition);
		var commit = CreateCommitLogRecord(transactionPosition + RecordOffset, transactionPosition,
