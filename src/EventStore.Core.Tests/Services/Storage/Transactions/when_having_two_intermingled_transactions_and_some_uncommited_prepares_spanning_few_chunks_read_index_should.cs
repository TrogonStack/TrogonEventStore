...
public class
	WhenHavingTwoIntermingledTransactionsAndSomeUncommitedPreparesSpanningFewChunksReadIndexShould<TLogFormat,
		TStreamId> :
	ReadIndexTestScenario<TLogFormat, TStreamId>
{
	...
	protected override async ValueTask WriteTestScenario(CancellationToken token)
	{
		...
		_t2CommitPos = await WriteCommit(t2.CorrelationId, t2.TransactionPosition, streamId2, _p2.EventNumber,
			token: token);
		_t1CommitPos = await WriteCommit(t1.CorrelationId, t1.TransactionPosition, streamId1, _p1.EventNumber,
			token: token);
	...
	{
		var res1 = await ReadIndex.ReadAllEventsBackward(new TFPos(_t2CommitPos, int.MaxValue), 100,
