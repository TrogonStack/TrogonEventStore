...
public class
	WhenBuildingAnIndexOffTfileWithPreparesAndCommits<TLogFormat, TStreamId> : ReadIndexTestScenario<TLogFormat,
	TStreamId>
{
	private Guid _id1;
	private Guid _id2;
	...
	protected override async ValueTask WriteTestScenario(CancellationToken token)
	{
		...
		var (_, pos1) = await Writer.Write(LogRecord.Prepare(_logFormat.RecordFactory, pos0, _id1, _id1, pos0, 0,
				streamId1, expectedVersion1++,
				PrepareFlags.SingleWrite, eventTypeId, new byte[0], new byte[0], DateTime.UtcNow),
			token);
		var (_, pos2) = await Writer.Write(LogRecord.Prepare(_logFormat.RecordFactory, pos1, _id2, _id2, pos1, 0,
				streamId2, expectedVersion2++,
				PrepareFlags.SingleWrite, eventTypeId, new byte[0], new byte[0], DateTime.UtcNow),
			token);
		var (_, pos3) = await Writer.Write(LogRecord.Prepare(_logFormat.RecordFactory, pos2, _id3, _id3, pos2, 0,
				streamId2, expectedVersion2++,
				PrepareFlags.SingleWrite, eventTypeId, new byte[0], new byte[0], DateTime.UtcNow),
			token);
