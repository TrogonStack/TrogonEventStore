...
{
	private static EnumeratorWrapper ReadAllForwardsFiltered(IPublisher publisher, Position position, IEventFilter filter = null)
	{
	...
	public class read_all_forwards_filtered<TLogFormat, TStreamId> : TestFixtureWithExistingEvents<TLogFormat, TStreamId>
	{
		...
		public async Task should_read_all_the_events()
		{
			await using var enumerator =
				ReadAllForwardsFiltered(_publisher, Position.Start, EventFilter.EventType.Prefixes(false, "type2"));
