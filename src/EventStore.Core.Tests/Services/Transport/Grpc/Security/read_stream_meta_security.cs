...
public class read_stream_meta_security<TLogFormat, TStreamId> : AuthenticationTestBase<TLogFormat, TStreamId>
{
	public static IEnumerable<TestCaseData> Cases()
	{
		yield return Case(StreamAclKind.UserOne, StatusCode.Unauthenticated,
			"reading_stream_meta_with_not_existing_credentials_is_not_authenticated", SecurityIdentity.Invalid);
		yield return Case(StreamAclKind.UserOne, StatusCode.PermissionDenied,
			"reading_stream_meta_with_no_credentials_is_denied", SecurityIdentity.Anonymous);
		yield return Case(StreamAclKind.UserOne, StatusCode.PermissionDenied,
			"reading_stream_meta_with_not_authorized_user_credentials_is_denied", SecurityIdentity.UserTwo);
		yield return Case(StreamAclKind.UserOne, StatusCode.OK,
			"reading_stream_meta_with_authorized_user_credentials_succeeds", SecurityIdentity.UserOne);
		yield return Case(StreamAclKind.UserOne, StatusCode.OK,
			"reading_stream_meta_with_admin_user_credentials_succeeds", SecurityIdentity.Admin);

		yield return Case(StreamAclKind.None, StatusCode.OK,
			"reading_no_acl_stream_meta_succeeds_when_no_credentials_are_passed", SecurityIdentity.Anonymous);
		yield return Case(StreamAclKind.None, StatusCode.Unauthenticated,
			"reading_no_acl_stream_meta_is_not_authenticated_when_not_existing_credentials_are_passed",
			SecurityIdentity.Invalid);
		yield return Case(StreamAclKind.None, StatusCode.OK,
			"reading_no_acl_stream_meta_succeeds_when_any_existing_user_credentials_are_passed",
			SecurityIdentity.UserOne, SecurityIdentity.UserTwo);
		yield return Case(StreamAclKind.None, StatusCode.OK,
			"reading_no_acl_stream_meta_succeeds_when_admin_user_credentials_are_passed", SecurityIdentity.Admin);

		yield return Case(StreamAclKind.All, StatusCode.OK,
			"reading_all_access_normal_stream_meta_succeeds_when_no_credentials_are_passed",
		...
		yield return Case(StreamAclKind.All, StatusCode.OK,
			...
			SecurityIdentity.UserOne, SecurityIdentity.UserTwo);
		yield return Case(StreamAclKind.All, StatusCode.OK,
			"reading_all_access_normal_stream_meta_succeeds_when_admin_user_credentials_are_passed",
			SecurityIdentity.Admin);
	}

	[TestCaseSource(nameof(Cases))]
	public async Task enforces_stream_metadata_read_security(
		StreamAclKind acl,
		...
		SecurityIdentity[] identities)
	{
		foreach (var identity in identities)
		{
			...
			AssertStatus(expectedStatus,
				await ExecuteOperation(SecurityOperation.ReadMetadata, streamName, identity));
		}
	}
