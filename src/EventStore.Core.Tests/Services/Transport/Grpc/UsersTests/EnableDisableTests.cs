...
public class EnableDisableTests
{
	...
	[TestFixture(typeof(LogFormat.V2), typeof(string))]
	public class when_disabling_user_without_admin_credentials<TLogFormat, TStreamId> : GrpcSpecification<TLogFormat, TStreamId>
	{
		private const string ActorLogin = "disable-denied-actor";
		private const string TargetLogin = "disable-denied-target";
	...
	[TestFixture(typeof(LogFormat.V2), typeof(string))]
	public class when_enabling_user_without_admin_credentials<TLogFormat, TStreamId> : GrpcSpecification<TLogFormat, TStreamId>
	{
		private const string ActorLogin = "enable-denied-actor";
		private const string TargetLogin = "enable-denied-target";
		private const string Password = "Pa55w0rd!";
		private UsersClient _client;
		private RpcException _exception;
		...
		protected override async Task Given()
		{
			...
			await _client.CreateAsync(CreateRequest(ActorLogin), GetCallOptions(AdminCredentials));
			await _client.CreateAsync(CreateRequest(TargetLogin), GetCallOptions(AdminCredentials));
			await _client.DisableAsync(DisableRequest(TargetLogin), GetCallOptions(AdminCredentials));
		}
