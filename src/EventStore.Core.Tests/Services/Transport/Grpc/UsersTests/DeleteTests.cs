...
public class DeleteTests {
	...
	[TestFixture(typeof(LogFormat.V2), typeof(string))]
	public class when_deleting_user_without_admin_credentials<TLogFormat, TStreamId> : GrpcSpecification<TLogFormat, TStreamId> {
		private const string LoginName = "delete-denied-user";
		...
		protected override async Task Given() {
			_client = new UsersClient(Channel);
			await _client.CreateAsync(CreateRequest(LoginName), GetCallOptions(AdminCredentials));
		}
