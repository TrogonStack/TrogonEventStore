...
public class when_truncating_database<TLogFormat, TStreamId> : SpecificationWithDirectoryPerTestFixture
{
	...
	public async Task everything_should_go_fine()
	{
		...
		var nodePort = miniNode.HttpEndPoint.Port;
		const int cnt = 50;
		...
		miniNode = new MiniNode<TLogFormat, TStreamId>(PathName, httpPort: nodePort);
		await miniNode.Start();
		await miniNode.Shutdown(keepDb: true);
	...
	public async Task with_truncate_position_in_completed_chunk_everything_should_go_fine()
	{
		...
		var miniNode = new MiniNode<TLogFormat, TStreamId>(PathName, chunkSize: chunkSize, cachedChunkSize: cachedSize);
		await miniNode.Start();
		...
		// --- first restart and truncation
		miniNode = new MiniNode<TLogFormat, TStreamId>(PathName, httpPort: nodePort, chunkSize: chunkSize,
