...
public abstract class ArchiveStorageWriterTests<T> : ArchiveStorageTestsBase<T>
{
	...
	public async Task can_store_a_chunk()
	{
		...
		using var archivedChunkContent =
			await CreateReaderSut(StorageType).GetChunk(destinationFile, CancellationToken.None);
	...
	public async Task throws_chunk_deleted_exception_if_local_chunk_doesnt_exist()
	{
		...
		await Assert.ThrowsAsync<ChunkDeletedException>(async () =>
			await sut.StoreChunk(localChunk, destinationFile, CancellationToken.None));
	}
