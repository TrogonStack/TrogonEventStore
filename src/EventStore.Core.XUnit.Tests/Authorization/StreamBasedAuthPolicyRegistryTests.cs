...
public class StreamBasedAuthPolicyRegistryTests
{
	...
	private StreamBasedAuthorizationPolicyRegistry CreateSut(
		...
		Action<ClientMessage.ReadStreamEventsBackward> onReadBackwards)
	{
	...
	public static TheoryData<DefaultSettingsTestCase> DefaultSettingsCases => new()
	{
		...
		new DefaultSettingsTestCase(FallbackPolicy, new AuthorizationPolicySettings(FallbackPolicy),
			[FallbackPolicy, AclsPolicy], null),
		new DefaultSettingsTestCase("valid", new AuthorizationPolicySettings("valid"), ["valid", AclsPolicy],
			new FakePolicySelectorPlugin("valid", true)),
		new DefaultSettingsTestCase("not found", new AuthorizationPolicySettings("not found"),
			[FallbackPolicy, AclsPolicy], new FakePolicySelectorPlugin("valid", true)),
		new DefaultSettingsTestCase("disabled", new AuthorizationPolicySettings("disabled"),
			[FallbackPolicy, AclsPolicy], new FakePolicySelectorPlugin("disabled", false)),
	};

	[Theory]
	[MemberData(nameof(DefaultSettingsCases))]
	public async Task when_starting_with_no_existing_settings_events_the_default_is_used(
		DefaultSettingsTestCase testCase)
	{
		var sut = CreateSut(
	...
	{
		new SettingsEventTestCase("disabled", shouldCallEnable: true, shouldCallDisable: false, [],
			new FakePolicySelectorPlugin("disabled", false)),
		new SettingsEventTestCase("valid", shouldCallEnable: true, shouldCallDisable: true, ["valid", AclsPolicy],
			new FakePolicySelectorPlugin("valid", true)),
		new SettingsEventTestCase(AclsPolicy, shouldCallEnable: false, shouldCallDisable: false, [AclsPolicy], null),
		new SettingsEventTestCase(FallbackPolicy, shouldCallEnable: false, shouldCallDisable: false,
			[FallbackPolicy, AclsPolicy], null),
		new SettingsEventTestCase("not found", shouldCallEnable: false, shouldCallDisable: false, [],
			new FakePolicySelectorPlugin("custom", true)),
		new SettingsEventTestCase("invalid", "invalid", "invalid", shouldCallEnable: false, shouldCallDisable: false,
			[], new FakePolicySelectorPlugin("invalid", true)),
		new SettingsEventTestCase("invalid json", SystemEventTypes.AuthorizationPolicyChanged, "invalid",
			shouldCallEnable: false, shouldCallDisable: false, [], new FakePolicySelectorPlugin("invalid json", true)),
		new SettingsEventTestCase("invalid event type", "invalid",
			JsonSerializer.Serialize(new AuthorizationPolicySettings(AclsPolicy)), shouldCallEnable: false,
			shouldCallDisable: false, [], new FakePolicySelectorPlugin("invalid event type", true)),
		new SettingsEventTestCase("empty json object", SystemEventTypes.AuthorizationPolicyChanged, "{}",
			shouldCallEnable: false, shouldCallDisable: false, [FallbackPolicy, AclsPolicy],
			new FakePolicySelectorPlugin("empty json object", true)),
		new SettingsEventTestCase("valid json without policyType", SystemEventTypes.AuthorizationPolicyChanged, "{}",
			shouldCallEnable: false, shouldCallDisable: false, [FallbackPolicy, AclsPolicy],
			new FakePolicySelectorPlugin("valid json without policyType", true)),
		new SettingsEventTestCase("empty policyType", SystemEventTypes.AuthorizationPolicyChanged, "{}",
			shouldCallEnable: false, shouldCallDisable: false, [FallbackPolicy, AclsPolicy],
			new FakePolicySelectorPlugin("valid json without policyType", true)),
	};
	...
	public async Task when_settings_events_exist_in_the_stream(SettingsEventTestCase testCase)
	{
		...
		var expectedPolicies = new[] { appliesPolicies, ["finished", AclsPolicy] };

		// Set up available plugins
		var availablePlugins =
			new Dictionary<string, FakePolicySelectorPlugin>
			{
	...
	public readonly struct SettingsEventTestCase
	{
		...
		public SettingsEventTestCase(string policyName, bool shouldCallEnable, bool shouldCallDisable,
			string[] appliesPolicies, FakePolicySelectorPlugin? customPlugin)
		{
		...
		public SettingsEventTestCase(string policyName, string eventType, string data, bool shouldCallEnable,
			bool shouldCallDisable, string[] appliesPolicies, FakePolicySelectorPlugin? customPlugin)
		{
			PolicyName = policyName;
			// EventNumber hardcoded to 2. Allows for a default and start event
