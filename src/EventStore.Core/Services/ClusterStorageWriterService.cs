	...
	IAsyncHandle<ReplicationMessage.DataChunkBulk>
{
	...
	async ValueTask IAsyncHandle<ReplicationMessage.ReplicaSubscribed>.HandleAsync(
		ReplicationMessage.ReplicaSubscribed message,
		CancellationToken token)
	{
		...
		if (message.SubscriptionPosition < writerCheck)
		{
			...
			{
				Log.Information(
					"Could not find a valid epoch to truncate to before position: {truncatePosition} (0x{truncatePosition:X})",
					message.SubscriptionPosition, message.SubscriptionPosition);
