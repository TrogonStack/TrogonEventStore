	...
	IAsyncDisposable
{
	...
	private async Task ProcessAsync(IPublisher publisher, ITelemetrySink sink, CancellationToken token)
	{
		...
		var scheduleInitialCollect =
			TimerMessage.Schedule.Create(InitialInterval, envelope, new TelemetryMessage.Collect());
		var scheduleCollect =
			TimerMessage.Schedule.Create(Interval - FlushDelay, envelope, new TelemetryMessage.Collect());
		...
		await foreach (var message in channel.Reader.ReadAllAsync(token))
		{
			switch (message)
			{
				...
				case TelemetryMessage.Response response:
					...
					data[response.Root] = new JsonObject { [response.Key] = response.Value };
					break;
