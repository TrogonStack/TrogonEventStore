...
public interface ITableIndex
{
	...
	ValueTask Scavenge(Func<IndexEntry, CancellationToken, ValueTask<bool>> shouldKeep, IIndexScavengerLog log,
...
public interface ITableIndex<TStreamId> : ITableIndex
{
	...
	ValueTask<IndexEntry?> TryGetLatestEntry(ulong stream, long beforePosition,
		Func<IndexEntry, CancellationToken, ValueTask<bool>> isForThisStream, CancellationToken token);

	ValueTask<IndexEntry?> TryGetLatestEntry(TStreamId streamId, long beforePosition,
		Func<IndexEntry, CancellationToken, ValueTask<bool>> isForThisStream, CancellationToken token);
