...
public class TFChunkManager : IThreadPoolWorkItem
{
	...
	public ValueTask<TFChunk.TFChunk> CreateTempChunk(ChunkHeader chunkHeader, int fileSize,
	...
	public async ValueTask<TFChunk.TFChunk> AddNewChunk(ChunkHeader chunkHeader,
		ReadOnlyMemory<byte> transformHeader, int fileSize, CancellationToken token)
	{
		...
		for (int i = chunkStartNumber; i <= chunkEndNumber; i += 1)
		{
			...
			if (!ReferenceEquals(previousRemovedChunk, oldChunk))
			{
				...
				if (previousRemovedChunk != null)
				{
					...
					Log.Information("{chunkExplanation} chunk #{oldChunk} is marked for deletion.",
						chunkExplanation,
