	...
	: ICalculator<TStreamId>
{
	...
	public async ValueTask Calculate(
		...
		var streamCalc = new StreamCalculator<TStreamId>(index, scavengePoint);
		var eventCalc = new EventCalculator<TStreamId>(chunkSize, index, state, scavengePoint, streamCalc);
	...
	private async ValueTask<(DiscardPoint DiscardPt, DiscardPoint MaybeDiscardPt, int CancellationCheckCounter)> CalculateDiscardPointsForOriginalStream(
		...
		while (true) {
			...
			var result = await index.ReadEventInfoForward(
				originalStreamHandle,
				fromEventNumber,
				maxCount,
				scavengePoint,
