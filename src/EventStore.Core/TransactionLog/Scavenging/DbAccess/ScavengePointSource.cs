...
public class ScavengePointSource(ILogger logger, IODispatcher ioDispatcher) : IScavengePointSource
{
		...
		CancellationToken cancellationToken)
	{
		...
		logger.Information("SCAVENGING: Latest scavenge point found is {scavengePoint}", scavengePoint);
		...
		CancellationToken cancellationToken)
	{
		...
		var payload = new ScavengePointPayload { Threshold = threshold, };

		var writeTcs = new TaskCompletionSource<bool>(
			TaskCreationOptions.RunContinuationsAsynchronously);
		ioDispatcher.WriteEvent(
			streamId: SystemStreams.ScavengePointsStream,
		...
		logger.Information("SCAVENGING: Added new scavenge point.");
