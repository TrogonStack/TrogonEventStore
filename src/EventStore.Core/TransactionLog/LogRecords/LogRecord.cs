...
public abstract class LogRecord : ILogRecord
{
		...
		switch (header.Type)
		{
			...
			case LogRecordType.System:
				if (header.Version > SystemLogRecord.SystemRecordVersion)
					return new LogV3EpochLogRecord(await LogV3Reader.ReadBytes(header.Type, header.Version, reader,
						length, token));
			...
			case LogRecordType.StreamWrite:
				return new LogV3StreamWriteRecord(await LogV3Reader.ReadBytes(header.Type, header.Version, reader,
					length, token));

			case LogRecordType.Stream:
				return new LogV3StreamRecord(await LogV3Reader.ReadBytes(header.Type, header.Version, reader, length,
					token));

			case LogRecordType.EventType:
				return new LogV3EventTypeRecord(await LogV3Reader.ReadBytes(header.Type, header.Version, reader, length,
					token));

			case LogRecordType.PartitionType:
				return new PartitionTypeLogRecord(await LogV3Reader.ReadBytes(header.Type, header.Version, reader,
					length, token));

			case LogRecordType.Partition:
				return new PartitionLogRecord(await LogV3Reader.ReadBytes(header.Type, header.Version, reader, length,
					token));
	...
	public static IPrepareLogRecord<TStreamId> Prepare<TStreamId>(IRecordFactory<TStreamId> factory, long logPosition,
		Guid correlationId, Guid eventId, long transactionPos,
	...
	public static IPrepareLogRecord<TStreamId> SingleWrite<TStreamId>(IRecordFactory<TStreamId> factory,
	...
	public static IPrepareLogRecord<TStreamId> TransactionBegin<TStreamId>(IRecordFactory<TStreamId> factory,
		long logPos, Guid correlationId, TStreamId eventStreamId,
	...
	public static IPrepareLogRecord<TStreamId> TransactionEnd<TStreamId>(IRecordFactory<TStreamId> factory, long logPos,
	...
	public static IPrepareLogRecord<TStreamId> DeleteTombstone<TStreamId>(IRecordFactory<TStreamId> factory,
		long logPosition, Guid correlationId, Guid eventId,
		TStreamId eventStreamId, TStreamId eventType, long expectedVersion,
