...
- The node endpoint carries database client APIs, the Admin UI, health probes, metrics, and other operator endpoints.
- The cluster endpoint carries node-to-node gossip, elections, follower-to-leader request forwarding, and database replication over gRPC.
...
cluster endpoint on the private cluster network and expose the node endpoint according
to client and operator requirements.

## HTTP configuration

HTTP/2 on the node endpoint carries gRPC traffic for database clients, while
regular HTTP routes serve the Admin UI,
...
Gossip, elections, follower-to-leader request forwarding, and database
replication use a dedicated HTTP/2 cluster listener so node-to-node traffic can
remain isolated from client and operator traffic.

The cluster listener binds to `ReplicationIp`:
...
The cluster listener uses `ReplicationPort`:
...
## Cluster endpoint advertisement

If the bound cluster address or port is not reachable as-is from the other
...
`ReplicationPortAdvertiseAs` for the gRPC cluster endpoint. If both are
...
Database client APIs, gossip, elections, replication, and follower-to-leader
forwarding use gRPC.
...
or TCP replication protocol. The separately configurable cluster listener is
HTTP/2-only and accepts only the internal gRPC services.
